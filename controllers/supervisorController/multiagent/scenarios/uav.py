...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        num_good_agents = 0
        num_uavs = 1
        num_agents = num_uavs + num_good_agents
        num_landmarks = 1
        # add agents
    ...
    def reset_world(self, world):
        ...
        # world.landmarks[1].color = np.array([0.85, 0.45, 0.25])
        ...
        for UAV in self.uavs(world):
            for t in UAV.target:
                if self.is_collision(UAV, t):
                    t.state.p_pos = np.random.uniform(-1, 1, world.dim_p)
                    break
    ...
    # return all agents that are not uavs
    ...
    def collects(self, world):
        return [a for a in world.landmarks if a.collector]

    def deposits(self, world):
        return [a for a in world.landmarks if not a.collector]
